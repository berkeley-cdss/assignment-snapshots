...
def request(
    ...
    course_endpoint: Annotated[
        str, typer.Option(help="Real OkPy course endpoint where the backups are actually stored, e.g. 'cal/cs88/sp25'")
    ] = None,
    limit: Annotated[
        int,
        typer.Option(
...
def store(
    ...
    sub_course_endpoint: Annotated[
        str, typer.Option(help="Substitute OkPy course endpoint (affects output file contents paths), e.g. 'cal/cs88/sp25'")
    ] = None,
    dump: Annotated[
        str,
        typer.Option(
    ...
    if sub_course_endpoint is None:
        course_endpoint = config_dict["okpy_api"].get("sub_course_endpoint", course_endpoint)
    else:
