...
function AssignmentsTable({ courseId, assignmentsData }) {
    ...
    const filteredAssignments = assignmentsData.filter(
        a => a.assignment.toLowerCase().includes(search.toLowerCase())
    );

    // TODO: replace with API call
    const columns = [
        { field: 'assignment', headerName: 'Assignment', flex: 2, headerClassName: 'column-header',
            renderCell: (params) => (
                <span
                    style={{ color: '#1976d2', cursor: 'pointer', textDecoration: 'underline' }}
                    onClick={() => navigate(`/courses/${courseId}/assignments/${params.row.id}`)}
                >
                    {params.value}
                </span>
            ),
